...
@SlingServlet(paths = "/bin/DDstatementcycle", methods = { "POST" }, metatype = true)
public class OPSSelectStatementCycleServlet extends SlingAllMethodsServlet {
	...
	protected void doPost(SlingHttpServletRequest request,
		...
		if (ldProductCode.length() > 0) {
			String getServiceURL = OPSConstants.WS_GET_STATEMENT_CYCLE_CODE_URL
					.replace("ldProductCode", ldProductCode).replace(
			...
			toReturnResults = newresponse.toString().replace("[",
					"[\"-1=Select\",");
		} else {
